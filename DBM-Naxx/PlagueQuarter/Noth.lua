...
local timerBlinkAfterTeleportBack = mod:NewNextTimer(10, 29208)
...
function mod:BackInRoom(delay)
	...
	warnTeleportSoon:Schedule(timer - 20)
	warnTeleportNow:Schedule(timer)
	timerBlinkAfterTeleportBack:Start()
	warnBlink:Schedule(6)
...
function mod:SPELL_AURA_APPLIED(args)
	if args:IsSpellID(29208, 29209, 29210, 29211) and args.sourceName == "Noth the Plaguebringer" then -- Blink
		announceBlink:Show()
		timerBlink:Start()
		warnBlink:Schedule(21)
	end
