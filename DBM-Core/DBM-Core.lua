...
-------------------------------
DBM = {
	Revision = ("$Revision: 18000 $"):sub(12, -3),
	Version = "18.0",
	DisplayVersion = "DBM-FotH v1.8-beta by Exorsized", -- the string that is shown as version
	ReleaseRevision = 18000 -- the revision of the latest stable version that is available (for /dbm ver2)
}
...
do
	...
	function DBM:COMBAT_LOG_EVENT_UNFILTERED(timestamp, event, sourceGUID, sourceName, sourceFlags, destGUID, destName, destFlags, ...)
		...
		args.originalSourceGUID = sourceGUID
		args.originalSourceName = sourceName
		args.originalSourceFlags = sourceFlags
...
SlashCmdList["DEADLYBOSSMODS"] = function(msg)
	...
	elseif cmd:sub(1, 10) == "collapsing" then
		...
		for token in string.gmatch(msg, "[^%s]+") do
		   if token_num == 2 then
		   	star_num = tonumber(token)
		   elseif token_num == 3 then
		   	player_name = token
		   elseif token_num > 3 then
		   	player_name = player_name.." "..token
		   end
...
do
	...
	function DBM:UpdateSpecialWarningOptions()
		...
		font:SetFont(DBM.Options.SpecialWarningFont, DBM.Options.SpecialWarningFontSize, "OUTLINE")
