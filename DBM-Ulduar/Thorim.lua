...
local specWarnHMTimerRanOut		= mod:NewSpecialWarning("hardmodePullTimerOver")
...
local timerFBVolley				= mod:NewCDTimer(9.5, 62604)
...
function mod:CHAT_MSG_MONSTER_YELL(msg)
	if msg == L.YellPhase2 or msg:find(L.YellPhase2) then		-- Bossfight (tank and spank)
		...
		enrageTimer:Stop()
		if self.Options.RangeFrame then
			DBM.RangeCheck:Show(8) -- only show radar in p2
		end
