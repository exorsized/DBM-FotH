...
local specWarnAnimusIn5			= mod:NewSpecialWarning("SpecialWarningAnimus", 3)
...
local timerMarkOfTheFacelessCD 	= mod:NewCDTimer(40, 63276)
local timerLifeLeech			= mod:NewTargetTimer(10, 63276)
local timerHardmode				= mod:NewTimer(251, "hardmodeSpawn")

local hardModeAvailable 		= true
...
function mod:OnCombatStart(delay)
	...
	specWarnAnimusIn5:Schedule(246-delay)
...
function mod:SPELL_DAMAGE(args)
	if args.destName == "Saronite Vapors" and hardModeAvailable then
		if saroniteVaporsDamage[args.destGUID] == nil then
			saroniteVaporsDamage[args.destGUID] = args.amount
		else
			saroniteVaporsDamage[args.destGUID] = saroniteVaporsDamage[args.destGUID] + args.amount
		end

		if (saroniteVaporsDamage[args.destGUID] >= 25199 and mod:IsDifficulty("heroic25")) or (saroniteVaporsDamage[args.destGUID] >= 12599 and not mod:IsDifficulty("heroic25")) then
			timerHardmode:Stop()
			specWarnAnimusIn5:Cancel()
