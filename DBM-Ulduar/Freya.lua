...
local timerTremorCD 		= mod:NewCDTimer(25, 62859)
local timerEonarsGiftCD     = mod:NewCDTimer(40, 62584)
local timerRootsCD      	= mod:NewCDTimer(12, 62439)

-- local specWarnBeamsSoon		= mod:NewSpecialWarning("WarningBeamsSoon", 3) -- Hard mode Sun Beam

local timerUnstableEnergyCD	= mod:NewCDTimer(15, 62451)

mod:AddBoolOption("HealthFrame", true)
mod:AddBoolOption("PlaySoundOnFury")
mod:AddBoolOption("SoundWarnCountingUnstableEnergy", false)
...
function mod:OnCombatStart(delay)
	...
	timerUnstableEnergyCD:Start()
	mod:CountdownFinalSeconds(self.Options.SoundWarnCountingUnstableEnergy, 15)
end
...
function mod:SPELL_CAST_START(args)
	if args:IsSpellID(62437, 62859) then
		specWarnTremor:Show()
		timerTremorCD:Start()
	elseif args:IsSpellID(62211, 62623, 62872, 64201) then -- Sunbeam useless af
...
function mod:SPELL_AURA_APPLIED(args)
	...
	elseif args:IsSpellID(62451, 62865) then -- Unstable Energy
		if GetTime() - lastUnstableEnergy > 13 then -- check for spam
			lastUnstableEnergy = GetTime()
			timerUnstableEnergyCD:Start()
			mod:CountdownFinalSeconds(self.Options.SoundWarnCountingUnstableEnergy, 15)
		end
...
function mod:SPELL_AURA_REMOVED(args)
	if args.spellId == 62519 then -- attuned nature remove, p2 start
		self.vb.phase = 2
        warnPhase2:Show()
